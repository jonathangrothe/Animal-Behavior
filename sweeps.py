...
h0_total_list = []
beta_list = [200] # 9.5: 0.215-0.32, 10: 0.208-0.325, 12: 0.189-0.339, 20: 0.177-0.338, 52: 0.182-0.325, 180: 0.189-0.326 ? 
base_h0_list = np.linspace(0.2192,0.2193,num=4) # 0.22 to 0.31 - 0.1925-0.335
...
for n in range(n_lines):
    for p in range(n_plots):
        ...
        for item in h0_range:
            h0_list.append([base_h0_list[n]+item,base_h0_list[n]+item,base_h0_list[n]+item])
...
for p in range(n_plots):
    ...
    figure_num, mean_time = sim_met.plot_metric(time_total_list[p::n_plots],h0_for_plot,colors_time,labels_time,figure_num,(8,8),
    ...
    for i in range(len(current_prob_list)):
        for p in range(ntargets):
            axes_flat[i].plot(h0_for_plot,current_prob_list[i][:,p],color=colors_prob[p], label=labels_prob[p])
            axes_flat[i].plot(h0_for_plot, np.add(current_prob_list[i][:,p],current_se_list[i][:,p]), color = colors_prob[p], linestyle = ':')
            axes_flat[i].plot(h0_for_plot, np.subtract(current_prob_list[i][:,p],current_se_list[i][:,p]), color = colors_prob[p], linestyle = ':')
