...
for a in range(nagents):
    initialx[a] = 50
    initialy[a] = 50
initialxt = [50,(50-15*np.sqrt(3)),(50+15*np.sqrt(3))]
initialyt = [80,35,35]
...
h0s = [0.22,0.22,0.22] # attraction vector, first are attraction for targets, then agents
h_b = 0.2
...
beta_sweep_list = np.linspace(40,200,num=17)
beta_h0range_list = []
beta_extrema = []

min_low = 0.2- 0.04 + 0.01*np.random.rand()
min_high = 0.2 + 0.04 + 0.01*np.random.rand()
max_low = 0.26 - 0.04 + 0.01*np.random.rand()
max_high = 0.26 + 0.04 + 0.01*np.random.rand()


for item in beta_sweep_list:
    ...
    print(f"boundaries, beta: {item}: min: {min_est} (range: {range_min}), max: {max_est} (range: {range_max})")
    beta_h0range_list.append(max_est-min_est)
    beta_extrema.append((min_est,max_est))
...
print(f"range list: {beta_h0range_list}")
ind_best_beta = np.argmax(beta_h0range_list)
best_beta = beta_sweep_list[ind_best_beta]

base['beta'] = best_beta
min_val = beta_extrema[ind_best_beta][0]+0.01
max_val = beta_extrema[ind_best_beta][1]-0.01
int_val = round((min_val+max_val)/2,5)

#min_val = 0.198
#max_val = 0.252
#int_val = (min_val+max_val)/2
h0_list = [[min_val,min_val,min_val],[int_val,int_val,int_val],[max_val,max_val,max_val]]
#print(h0_list)
...
for s in range(n_plots):
    ...
    axs0[s].set_title(f"{"allocentric" if allocentricFlag==1 else "egocentric"}, h0: {h0_list[s]}, beta: {beta}")
